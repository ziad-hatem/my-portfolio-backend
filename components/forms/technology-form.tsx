...
interface TechnologyFormProps {
  initialData?: TechnologyFormValues & { id?: string }
  isEditing?: boolean
}

export default function TechnologyForm({ initialData, isEditing = false }: TechnologyFormProps) {
  const router = useRouter()
  const [isLoading, setIsLoading] = useState(false)

  const defaultValues: Partial<TechnologyFormValues> = {
  ...
  const onSubmit = async (data: TechnologyFormValues) => {
    ...
    try {
      ...
      if (isEditing && initialData?.id) {
        const result = await updateTechnology(initialData.id, formData)
        if (result.success) {
          toast({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{isEditing ? "Edit Technology" : "Create Technology"}</CardTitle>
        <CardDescription>
          {isEditing ? "Update technology information" : "Add a new technology to the system"}
        </CardDescription>
      </CardHeader>
      <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>This is the name of the technology</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Upload an icon for this technology</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
